...
Word Embeddings are dense, low-dimension vector representations of words. These vectors can be learned with neural networks. The basic idea is to train an neural network on a suitable task and then use the columns of the weight matrix of a hidden layer (called \emph{embedding matrix}) as word embeddings. The embedding matrix has the size of $(\text{number of words in the vocabulary } \mathcal{V} \times\allowbreak \text{number of neurons }\allowbreak \text{in the hidden layer})$, so that each column represents one word in $\mathcal{V}$ and each embedding has as many components as the hidden layer has neurons. This leads to word vectors which are much smaller than $|\mathcal{V}|$.
