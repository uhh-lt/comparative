...
\cite{Habernal2014Argumentation-M} presented a comparison between the results of two different annotation studies. One used the Claim/Premise-Model, while the other one used the Toulmin model. They emphasized that there is no "one-size-fits-all" model. (ERWEITERN)
...
\label{img:diffen}
\caption{\enquote{Hamburg vs. Berlin} on Diffen}
\end{figure}

\begin{figure}[h]
\includegraphics[width=1\textwidth]{images/ds-sys/versus}
\label{img:versus}
\caption{\enquote{Hamburg vs. Berlin} on Versus}
\end{figure}
